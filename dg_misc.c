		 ...
		 int type, char *cmd)
{
  ...
  if (IS_SET(SINFO.targets, TAR_IGNORE)) {
    ...
    if (!target && IS_SET(SINFO.targets, TAR_CHAR_ZONE))
      if (((tch = get_char(t)) != NULL) && (world[IN_ROOM(caster)].zone == world[IN_ROOM(tch)].zone))
	target = TRUE;
...
int valid_dg_target(struct char_data *ch, int bitvector)
{
  ...
  else if (GET_LEVEL(ch) < LVL_SAINT) 
  ...
  else if (!PRF_FLAGGED(ch, PRF_NOHASSLE) && GET_INVIS_LEV(ch) < LVL_SAINT)
