...
/* removed by anubis to get rid abbreviations
int isname(const char *str, const char *namelist)
{
  char *newlist;
  char *curtok;
...
void affect2_modify(struct char_data *ch, byte loc, sbyte mod, 
                   bitvector_t bitv, bool add)
{
  if (add)
...
void affect_total(struct char_data *ch)
{
  ...
  for (af = ch->affected2; af; af = af->next)
    affect2_modify(ch, af->location, af->modifier, af->bitvector, FALSE);
  ...
  for (af = ch->affected2; af; af = af->next)
    affect2_modify(ch, af->location, af->modifier, af->bitvector, TRUE);

  /* Make certain values are between 0..25, not < 0 and not > 25! */

  i = (IS_NPC(ch) || GET_LEVEL(ch) >= LVL_GOD) ? 25 : 18;
...
void affect2_to_char(struct char_data *ch, struct affected_type *af)
{
  struct affected_type *affected_alloc;
...
void affect2_remove(struct char_data *ch, struct affected_type *af)
{
  struct affected_type *temp;
...
void affect2_from_char(struct char_data *ch, int type)
{
  struct affected_type *hjp, *next;

  for (hjp = ch->affected2; hjp; hjp = next) {
    next = hjp->next;
    if (hjp->type == type)
      affect2_remove(ch, hjp);
  }
}
...
bool affected2_by_spell(struct char_data *ch, int type)
{
  struct affected_type *hjp;
...
void affect2_join(struct char_data *ch, struct affected_type *af,
		      bool add_dur, bool avg_dur, bool add_mod, bool avg_mod)
{
  struct affected_type *hjp, *next;
  bool found = FALSE;

  for (hjp = ch->affected2; !found && hjp; hjp = next) {
    next = hjp->next;
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
  ...
  if (IN_ROOM(obj) != NOWHERE) {
    ...
    if (invalid_align(ch, obj) && GET_LEVEL(ch) < LVL_DEITY) { // Higher imms don't get zapped.
