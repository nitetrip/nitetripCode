...
struct board_info_type board_info[NUM_OF_BOARDS] = {
  {500, LVL_SAINT, LVL_SAINT, LVL_GOD, LIB_ETC "board.immort", 0},
  {512, LVL_IMPL, LVL_IMPL, LVL_IMPL, LIB_ETC "board.impl", 0},
  {513, LVL_GOD, LVL_GOD, LVL_GOD, LIB_ETC "board.grgod", 0},
  {529, LVL_SAINT, LVL_SAINT, LVL_DEITY, LIB_ETC "board.todo.0", 0},
  {20500, LVL_SAINT, LVL_SAINT, LVL_GOD, LIB_ETC "board.academy", 0},
  {4000, LVL_BUILDER, LVL_BUILDER, LVL_COUNT, LIB_ETC "board.suggestion", 0},
  {4001, 2, 2, LVL_GOD, LIB_ETC "board.social", 0},
  {524, LVL_LIEGE, LVL_LIEGE, LVL_GOD, LIB_ETC "board.liege", 0},
  {2599, LVL_COUNT, LVL_COUNT, LVL_GOD, LIB_ETC "board.count", 0},
  {4299, LVL_DEITY, LVL_DEITY, LVL_IMPL, LIB_ETC "board.deity", 0},
  {10200, 2, 2, LVL_GOD, LIB_ETC "board.midsuggestion", 0},
  {10201, 2, 2, LVL_GOD, LIB_ETC "board.midsocial", 0},
  {582, 2, 2, LVL_DEITY, LIB_ETC "board.coder", 0},
 };
...
int Board_show_board(int board_type, struct char_data *ch, char *arg, struct obj_data *board)
{
  ...
  if (!num_of_msgs[board_type])
    ...
    for (i = num_of_msgs[board_type] - 1; i >= 0; i--) {
      ...
      nlen = snprintf(buf + len, sizeof(buf) - len, "%-2d : %s%s\r\n", num_of_msgs[board_type] - i, MSG_HEADING(board_type, i), !(msg_storage[MSG_SLOTNUM(board_type, i)]) ? " (empty)" : "");
    ...
    for (i = 0; i < num_of_msgs[board_type]; i++) {
      ...
      nlen = snprintf(buf + len, sizeof(buf) - len, "%-2d : %s%s\r\n", i + 1, MSG_HEADING(board_type, i), !(msg_storage[MSG_SLOTNUM(board_type, i)]) ? " (empty)" : "");
      if (len + nlen >= sizeof(buf) || nlen < 0)
        break;
      len += nlen;
    }
...
/* temporarily remove the higher level check, as a convenience until I've
 * gotten through a bunch of the minor tweaks on the Todo list  CJW 12/03/06
 */
/*
