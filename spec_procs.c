...
return;
  ...
  */
  ...
  if (value) {
    ...
    if (GET_LEVEL(ch) < 3)
	  {
    sprintf(buf, "You are awarded %d experience for outstanding performance.\r\n", value);
	send_to_char(ch, buf);
	gain_exp(ch, value);
	  }
	else
	  {
	if ( value != 1 ) {
		sprintf(buf, "You are awarded %d coins for outstanding performance.\r\n", value);
		send_to_char(ch, buf);
	}
...
/**** Does the Player have the Prerequisites? ****/
 if (spell_info[skill_num].first_prereq[(int) GET_CLASS(ch)]!= TYPE_UNDEFINED)
 {
 /* is there a first prereq? if so check they have practiced it */
	...
	if (spell_info[skill_num].second_prereq[(int) GET_CLASS(ch)]!= TYPE_UNDEFINED)
	{
...
SPECIAL(snake)
{
  ...
  call_magic(ch, FIGHTING(ch), 0, NOWHERE, SPELL_POISON, GET_LEVEL(ch), CAST_SPELL);
  return (TRUE);
}
