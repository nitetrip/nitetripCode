...
void sedit_shop_flags_menu(struct descriptor_data *d)
{
  ...
  sprintbit(S_BITVECTOR(OLC_SHOP(d)), shop_bits, bitbuf, sizeof(bitbuf));
...
void sedit_no_trade_menu(struct descriptor_data *d)
{
  ...
  sprintbit(S_NOTRADE(OLC_SHOP(d)), trade_letters, bitbuf, sizeof(bitbuf));
  write_to_output(d, "\r\nCurrently won't trade with: %s%s%s\r\n"
	  "Enter choice : ", cyn, bitbuf, nrm);
...
{
  char tbitbuf[MAX_INPUT_LENGTH], sbitbuf[MAX_INPUT_LENGTH];
  struct shop_data *shop;
  ...
  clear_screen(d);
  sprintbit(S_NOTRADE(shop), trade_letters, tbitbuf, sizeof(tbitbuf));
  sprintbit(S_BITVECTOR(shop), shop_bits, sbitbuf, sizeof(sbitbuf));
  write_to_output(d,
	  "-- Shop Number : [%s%d%s]\r\n"
...
/*-------------------------------------------------------------------*/
  ...
  case SEDIT_KEEPER:
     ...
     */
    /* going to remove this to try to fix the bug
