...
int find_door(struct char_data *ch, const char *type, char *dir, const char *cmdname)
{
  ...
  if (*dir) {			/* a direction was specified */
	...
	if (isname(type, EXIT(ch, door)->keyword))
	  return (door);
	else { /*Changed to match hidden door wrong keyword with no keyword no hidden door - mak */
...
#define DOOR_KEY(ch, obj, door)		((obj) ? (GET_OBJ_VAL(obj, 2)) : \
					(EXIT(ch, door)->key)) /*moved from the lower section*/
...
ACMD(do_gen_door)
{
  ...
  if ((obj) || (door >= 0)) {
    ...
    else if (!has_key(ch, keynum) && (GET_LEVEL(ch) < LVL_DEITY) &&
...
ACMD(do_wake)
{
  ...
  if (*arg) {
    ...
    else if (AFF_FLAGGED(vict, AFF_SLEEP) || AFF_FLAGGED(vict, 
AFF_HEALING_DREAM) )
  ...
  if (AFF_FLAGGED(ch, AFF_SLEEP) || AFF_FLAGGED(ch, AFF_HEALING_DREAM) )
