...
ACMD(do_attach) 
{
  ...
  if (is_abbrev(arg, "mobile")) {
    if ((victim = get_char_vis(ch, targ_name, NULL, FIND_CHAR_WORLD))) {
      if (IS_NPC(victim))  {
...
{  
  ...
  if (!str_cmp(arg1, "room")) {
    ...
    if (is_abbrev(arg1, "mobile")) {
      if (!(victim = get_char_vis(ch, arg2, NULL, FIND_CHAR_WORLD)))
...
void script_vlog(const char *format, va_list args) 
{
  ...
  for (i = descriptor_list; i; i = i->next) {
    ...
    send_to_char(i->character, "%s%s%s", CCCYN(i->character, C_NRM), output, CCNRM(i->character, C_NRM));
  }
}
  ...
  if (!*field) {
	...
	*str = '\0';
	...
	if (!str_cmp(field, "char")) {
	  ...
	  sprintf(str, "%d", ((num = atoi(field)) > 0) ? rand_number(1, num) : 0);
	...
	sprinttype(GET_CLASS(c), pc_class_types, str, sizeof(str));
	  ...
	  sprinttype(GET_RACE(c), pc_race_types, str, sizeof(str));
        }
      }
#endif
...
/* see note in dg_scripts.h */
	...
	sprinttype(GET_OBJ_TYPE(o), item_types, str, sizeof(str));
        ...
        if (obj_room(o) != NOWHERE)
          sprintf(str,"%c%ld",UID_CHAR, (long)obj_room(o) + ROOM_ID_BASE);
        else strcpy(str, "");
...
int script_driver(void *go, trig_data *trig, int type, int mode)
{
  ...
  for (cl = (mode == TRIG_NEW) ? trig->cmdlist : trig->curr_state;
	...
	process_detach(go, sc, trig, type, cmd);
        ...
        mudlog(NRM, LVL_DEITY, TRUE, "%s", DG_SCRIPT_VERSION);
...
ACMD(do_tlist)
{
  ...
  if (!*buf) {
    send_to_char(ch, "Usage: tlist <beginning number or zone> [<ending number>]\r\n");
    return;
  }
