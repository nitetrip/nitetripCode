...
void auto_equip(struct char_data *ch, struct obj_data *obj, int location)
{
  ...
  if (location > 0) {	/* Was wearing it. */
	 ...
	 * zapped through the auto-equipping.
        ...
        mudlog(BRF, LVL_SAINT, TRUE, "SYSERR: autoeq: '%s' already equipped in position %d.", GET_NAME(ch), location);
        location = LOC_INVENTORY;
      }
    }
  }
...
int Crash_load(struct char_data *ch)
{
  ...
  if (!(fl = fopen(filename, "r+b"))) {
    ...
    mudlog(NRM, MAX(LVL_SAINT, GET_INVIS_LEV(ch)), TRUE, "%s entering game with no equipment.", GET_NAME(ch));
  ...
  if (rent.rentcode == RENT_RENTED || rent.rentcode == RENT_TIMEDOUT) {
    ...
    if (cost > GET_GOLD(ch) + GET_BANK_GOLD(ch)) {
      fclose(fl);
      mudlog(BRF, MAX(LVL_SAINT, GET_INVIS_LEV(ch)), TRUE, "%s entering game, rented equipment lost (no $).", GET_NAME(ch));
      Crash_crashsave(ch);
  ...
  case RENT_RENTED:
    mudlog(NRM, MAX(LVL_SAINT, GET_INVIS_LEV(ch)), TRUE, "%s un-renting and entering game.", GET_NAME(ch));
    break;
  case RENT_CRASH:
    mudlog(NRM, MAX(LVL_SAINT, GET_INVIS_LEV(ch)), TRUE, "%s retrieving crash-saved items and entering game.", GET_NAME(ch));
    break;
  case RENT_CRYO:
    mudlog(NRM, MAX(LVL_SAINT, GET_INVIS_LEV(ch)), TRUE, "%s un-cryo'ing and entering game.", GET_NAME(ch));
    break;
  case RENT_FORCED:
  case RENT_TIMEDOUT:
    mudlog(NRM, MAX(LVL_SAINT, GET_INVIS_LEV(ch)), TRUE, "%s retrieving force-saved items and entering game.", GET_NAME(ch));
    break;
  default:
    mudlog(BRF, MAX(LVL_SAINT, GET_INVIS_LEV(ch)), TRUE,
  ...
  mudlog(NRM, MAX(GET_INVIS_LEV(ch), LVL_DEITY), TRUE, "%s (level %d) has %d object%s (max %d).",
		         ...
		         int cmd, char *arg, int mode)
{
  ...
  if (CMD_IS("rent")) {
    ...
    if (mode == RENT_FACTOR) {
      ...
      mudlog(NRM, MAX(LVL_SAINT, GET_INVIS_LEV(ch)), TRUE, "%s has rented (%d/day, %d tot.)",
      ...
      mudlog(NRM, MAX(LVL_SAINT, GET_INVIS_LEV(ch)), TRUE, "%s has cryo-rented.", GET_NAME(ch));
      SET_BIT(PLR_FLAGS(ch), PLR_CRYO);
    }
