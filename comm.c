...
void game_loop(socket_t mother_desc)
{
  ...
  while (!circle_shutdown) {
    ...
    if (reread_wizlist) {
      ...
      mudlog(CMP, LVL_IMPL, TRUE, "Signal received - rereading wizlists.");
      reboot_wizlists();
    }
    if (emergency_unban) {
      emergency_unban = FALSE;
      mudlog(BRF, LVL_IMPL, TRUE, "Received SIGUSR2 - completely unrestricting game (emergent)");
      ban_list = NULL;
      circle_restrict = 0;
      num_invalid = 0;
    }
...
void heartbeat(int heart_pulse)
{
  ...
  if (!(heart_pulse % PULSE_PAIN_ROOM))
    check_pain_room();
...
char *make_prompt(struct descriptor_data *d)
{
  ...
  if (d->showstr_count) {
    ...
    *prompt = '\0';
    ...
    if (PRF_FLAGGED(d->character, PRF_DISPHP) && len < sizeof(prompt)) {       
      count = snprintf(prompt + len, sizeof(prompt) - len, "%s%dH%s ", CCBRD(d->character, C_NRM),
    ...
    if (PRF_FLAGGED(d->character, PRF_DISPMANA) && len < sizeof(prompt)) {      
      count = snprintf(prompt + len, sizeof(prompt) - len, "%s%dM%s ", CCBGR(d->character, C_NRM),
    ...
    if (PRF_FLAGGED(d->character, PRF_DISPMOVE) && len < sizeof(prompt)) {     
      count = snprintf(prompt + len, sizeof(prompt) - len, "%s%dV%s ", CCBYL(d->character, C_NRM),
    ...
    if (PRF_FLAGGED(d->character, PRF_DISPEXP) && len < sizeof(prompt)) {
        if (GET_LEVEL(d->character) > LVL_SAINT) {
            count = snprintf(prompt + len, sizeof(prompt) - len, "%s%dX%s ", CCBCN(d->character, C_NRM),
            (-1), CCNRM(d->character, C_NRM));}
        else {
            count = snprintf(prompt + len, sizeof(prompt) - len, "%s%dX%s ", CCBCN(d->character, C_NRM),
     ...
     if (PRF_FLAGGED(d->character, PRF_DISPTARGET) && len < sizeof(prompt)
      && FIGHTING(d->character) ) 
     {
      	  
  switch (GET_POS(FIGHTING(d->character))) {
  case POS_MORTALLYW:
          count = snprintf(prompt + len, sizeof(prompt) - len, "%sMortally Wounded%s ", CCBRD(d->character, C_NRM),
          CCNRM(d->character, C_NRM)); 
      if (count >= 0)
        len += count;
	break;
  case POS_INCAP:
          count = snprintf(prompt + len, sizeof(prompt) - len, "%sIncapacitated%s ", CCBRD(d->character, C_NRM),
          CCNRM(d->character, C_NRM)); 
      if (count >= 0)
        len += count;
	break;
  case POS_STUNNED:
          count = snprintf(prompt + len, sizeof(prompt) - len, "%sStunned%s ", CCBRD(d->character, C_NRM),
          CCNRM(d->character, C_NRM)); 
      if (count >= 0)
        len += count;
	break;
  case POS_DEAD:
          count = snprintf(prompt + len, sizeof(prompt) - len, "%sDead%s ", CCBRD(d->character, C_NRM),
          CCNRM(d->character, C_NRM)); 
      if (count >= 0)
        len += count;
	break;
  default:
	  ...
	  percent = (100 * GET_HIT(FIGHTING(d->character))) / GET_MAX_HIT(FIGHTING(d->character));
        ...
        percent = -1;
      if (percent >= 100)
          count = snprintf(prompt + len, sizeof(prompt) - len, "%sExcellent condition%s ", CCGRN(d->character, C_NRM),
          CCNRM(d->character, C_NRM)); 
      else if (percent >= 90)
          count = snprintf(prompt + len, sizeof(prompt) - len, "%sA few scratches%s ", CCGRN(d->character, C_NRM),
          CCNRM(d->character, C_NRM)); 
      else if (percent >= 75)
          count = snprintf(prompt + len, sizeof(prompt) - len, "%sSmall wounds and bruises%s ", CCGRN(d->character, C_NRM),
          CCNRM(d->character, C_NRM)); 
      else if (percent >= 50)
          count = snprintf(prompt + len, sizeof(prompt) - len, "%sQuite a few wounds%s ", CCBYL(d->character, C_NRM),
          CCNRM(d->character, C_NRM)); 
      else if (percent >= 30)
          count = snprintf(prompt + len, sizeof(prompt) - len, "%sBig nasty wounds%s ", CCBYL(d->character, C_NRM),
          CCNRM(d->character, C_NRM)); 
      else if (percent >= 15)
          count = snprintf(prompt + len, sizeof(prompt) - len, "%sPretty hurt%s ", CCBRD(d->character, C_NRM),
          CCNRM(d->character, C_NRM)); 
      else if (percent > 0)
...
int new_descriptor(socket_t s)
{
  ...
  if (isbanned(newd->host) == BAN_ALL) {
    CLOSE_SOCKET(desc);
    mudlog(CMP, LVL_DEITY, TRUE, "Connection attempt denied from [%s]", newd->host);
    free(newd);
    return (0);
  }
#if 0
   ...
   */
  mudlog(CMP, LVL_DEITY, FALSE, "New connection from [%s]", newd->host);
#endif
...
void close_socket(struct descriptor_data *d)
{
  ...
  if (d->character) {
    ...
    if (IS_PLAYING(d) || STATE(d) == CON_DISCONNECT) {
      ...
      save_char(link_challenged, GET_LOADROOM(link_challenged));
      mudlog(NRM, MAX(LVL_DEITY, GET_INVIS_LEV(link_challenged)), TRUE, "Closing link to: %s.", GET_NAME(link_challenged));
      clanlog(link_challenged, "%s has lost link.", GET_NAME(link_challenged));
    } else {
      mudlog(NRM, LVL_DEITY, TRUE, "Losing player: %s. [%s]", GET_NAME(d->character) ? GET_NAME(d->character) : "<null>", GET_LEVEL(d->character) >= LVL_GOD ? "Hostname Protected" : d->host);
      free_char(d->character);
    }
  } else
    mudlog(NRM, LVL_IMPL, TRUE, "Losing descriptor without char. [%s]", d->host );
