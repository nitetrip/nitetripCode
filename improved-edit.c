...
void format_text(char **ptr_string, int mode, struct descriptor_data *d, unsigned int maxlen)
{
  ...
  int space_count, new_para = FALSE, first_indent = TRUE;
  ...
  while (*flow) {
  /* strip out whitespace -- handle special case where three spaces indicates an indentation  -tye */
  space_count = 0;
    while (*flow && strchr("\n\r\f\t\v ", *flow))
    {
        if (strchr(" ", *flow))
...
/* add the next word to the formatted output */
