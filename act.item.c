...
*  Usage: object handling routinndow- get/drop and container handling     *
*                                                                         *
...
void perform_remove(struct char_data *ch, int pos, int display);
...
int can_take_obj(struct char_data *ch, struct obj_data *obj)
{
  if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch)) {
    ...
    return (0);
  } else if ((IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj)) > CAN_CARRY_W(ch) && (GET_LEVEL(ch) < LVL_IMPL)) {
    act("$p: you can't carry that much weight.", FALSE, ch, obj, 0, TO_CHAR);
    return (0);
  } else if (!(CAN_WEAR(obj, ITEM_WEAR_TAKE)) && (GET_LEVEL(ch) < LVL_IMPL)) {
				     ...
				     struct obj_data *cont, int mode)
{
  if (mode == FIND_OBJ_INV || can_take_obj(ch, obj)) {
    ...
    /*This is now checked in the can_take_obj() 
    * if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch)) 
    *  act("$p: you can't hold any more items.", FALSE, ch, obj, 0, TO_CHAR); 
    */
        else if (get_otrigger(obj, ch) && (obj) && can_take_obj(ch, obj)) { /* obj may be purged */
      obj_from_obj(obj);
      obj_to_char(obj, ch);
      act("You get $p from $P.", FALSE, ch, obj, cont, TO_CHAR);
      act("$n gets $p from $P.", TRUE, ch, obj, cont, TO_ROOM);
      if (OBJ_FLAGGED(obj, ITEM_LESSER_HIDDEN)) {
       REMOVE_BIT(GET_OBJ_EXTRA(obj), ITEM_LESSER_HIDDEN);
      act("As you touch $p it becomes more defined!", FALSE, ch, obj, 0, TO_CHAR);
      act("As $n gets $p from $P it becomes more defined!", TRUE, ch, obj, cont, TO_ROOM);
      }
  ...
  if (OBJVAL_FLAGGED(cont, CONT_CLOSED))
    ...
    if (!(obj = get_obj_in_list_vis(ch, arg, NULL, cont->contains))) {
      ...
      while (obj && howmany--) {
        ...
        /* added by anubis for claim */
	    if(cont->corpse_id > 0 && GET_IDNUM(ch) != cont->corpse_id && GET_OBJ_VNUM(obj) > 0 && GET_LEVEL(ch) < LVL_GOD) {
	       send_to_char(ch, "You are not allowed to touch that corpse!\r\n");
	       return;
	    }
    ...
    for (obj = cont->contains; obj; obj = next_obj) {
      next_obj = obj->next_content;
      if (CAN_SEE_OBJ(ch, obj) && ((!OBJ_FLAGGED(obj, ITEM_GREATER_HIDDEN)) && (!OBJ_FLAGGED(obj, ITEM_LESSER_HIDDEN))) &&
...
int perform_get_from_room(struct char_data *ch, struct obj_data *obj)
{
   ...
   /* added by anubis for claim */
	    if(obj->corpse_id > 0 && GET_IDNUM(ch) != obj->corpse_id && GET_LEVEL(ch) < LVL_GOD) {
	       send_to_char(ch, "You are not allowed to touch that corpse!\r\n");
	       return(0);
	    }	   
  if (can_take_obj(ch, obj) &&
      get_otrigger(obj, ch) &&
      !trap_check(ch, GET_OBJ_VNUM(obj)) &&  /* Anubis */
    ...
    if (OBJ_FLAGGED(obj, ITEM_LESSER_HIDDEN)) {
       REMOVE_BIT(GET_OBJ_EXTRA(obj), ITEM_LESSER_HIDDEN);
      act("As you touch $p it becomes more defined!", FALSE, ch, obj, 0, TO_CHAR);
      act("As $n gets $p it becomes more defined!", TRUE, ch, obj, 0, TO_ROOM);
      }
    get_check_money(ch, obj);
    return (1);
  }
...
#define VANISH(mode) ((mode == SCMD_DONATE) ? "  It vanishes in a puff of smoke!" : "")
...
*/
  ...
  if (amount && (subcmd == SCMD_JUNK)) {
	...
	if ( amount != 1 ) { 
		sprintf(buf, "You have been rewarded %d coins.\r\n", amount);
		send_to_char(ch, buf);
	}

	else if ( amount == 1 )	{
		sprintf(buf, "You have been rewarded %d coin.\r\n", amount); 
		send_to_char(ch, buf);
	}
		       ...
		       struct obj_data *obj)
{
 ...
 if (GET_LEVEL(vict) < GET_OBJ_LEVEL(obj)) {
...
ACMD(do_drink)
{
  ...
  /*
  * Changing amount to 1, rather than whatever the hell they were thinking.
...
ACMD(do_eat)
{
  ...
  if ((GET_OBJ_TYPE(food) != ITEM_FOOD) && (GET_LEVEL(ch) < LVL_DEITY)) {
  ...
  if (GET_OBJ_VAL(food, 3) && (GET_LEVEL(ch) < LVL_SAINT)) {
...
/*
 * Sets the PC to his default racial size
 * then removes/sets modifiers he is wearing
 * to determine size
 */
...
struct obj_data *obj;
      ...
      for (i = 0; i < NUM_WEARS; i++)
      	if (GET_EQ(ch, i) && ((GET_SIZE(ch) + 1) < GET_OBJ_SIZE(GET_EQ(ch, i))) && (GET_OBJ_SIZE(GET_EQ(ch, i)) != SIZE_SPECIAL) && ((GET_SIZE(ch) != SIZE_SPECIAL)))
        /* Check for SIZE_SPECIAL of player added - mak 11.30.04 */
          {
          obj = GET_EQ(ch, i);
	  act("You are suddendly too small and $p slips off you!", FALSE, ch, GET_EQ(ch, i), 0, TO_CHAR);
	  perform_remove(ch, i, 0);
	  obj_from_char(obj);
	  obj_to_room(obj, IN_ROOM(ch));
          /* Fix the PCs size first if it needs fixing */
          if ((GET_SIZE(ch) != SIZE_SPECIAL)) 
            {
            fix_size(ch);
            }
        }
	...
	if (GET_EQ(ch, i) && ((GET_SIZE(ch) - 1) > GET_OBJ_SIZE(GET_EQ(ch, i))) && (GET_OBJ_SIZE(GET_EQ(ch, i)) != SIZE_SPECIAL) && ((GET_SIZE(ch) != SIZE_SPECIAL))) {
	  obj = GET_EQ(ch, i);
	  act("$p suddenly bursts off as you grow larger!", FALSE, ch, GET_EQ(ch, i), 0, TO_CHAR);
	  perform_remove(ch, i, 0);
          obj_from_char(obj);
          obj_to_room(obj, IN_ROOM(ch));
          /* Fix the PCs size first if it needs fixing */
...
void perform_wear(struct char_data *ch, struct obj_data *obj, int where)
{
  ...
  /* Item too large to use ? */
  if ((GET_SIZE(ch) + 1) < (GET_OBJ_SIZE(obj)) && ((GET_OBJ_SIZE(obj) != SIZE_SPECIAL) && ((GET_SIZE(ch) != SIZE_SPECIAL))))
   {
     act("$p is too large for you to use.", FALSE, ch, obj, 0, TO_CHAR);
     return;
     }

  /* Item too small to use ? */
  if ((GET_SIZE(ch) - 1) > GET_OBJ_SIZE(obj) && (GET_OBJ_SIZE(obj) != SIZE_SPECIAL) && ((GET_SIZE(ch) != SIZE_SPECIAL))) {
     act("$p is too small for you to use.", FALSE, ch, obj, 0, TO_CHAR);
     return;
     }

  /* Can class use the item ? */
  if (invalid_class(ch, obj))
  {
    act("You cannot understand what to do with $p.", FALSE, ch, obj, 0, TO_CHAR);
    act("$n cannot understand what to do with $p.", FALSE, ch, obj, 0, TO_ROOM);
    return;
  }

  /* Can race use the item ? */
  if (invalid_race(ch, obj))
  {
    act("Something about you resists $p.", FALSE, ch, obj, 0, TO_CHAR);
    act("Something about $n resists $p.", FALSE, ch, obj, 0, TO_ROOM);
    return;
  }


   /* Limit 1 of the same obj to be worn at a time */
   int i;
   for (i = 0; i < NUM_WEARS; i++) {
     if ((GET_EQ(ch, i)) && GET_OBJ_VNUM(GET_EQ(ch, i)) == GET_OBJ_VNUM(obj))
     { act("You already have $p equipped somewhere else!.", FALSE, ch, obj, 0, TO_CHAR);
     return; }
    }
  ...
  if (!wear_otrigger(obj, ch, where))
    return;
  if (!OBJ_FLAGGED(obj, ITEM_NO_EQ_MSG))
...
ACMD(do_wear)
{
  ...
  if (dotmode == FIND_ALL) {
    ...
    else if ((GET_LEVEL(ch) > GET_OBJ_MAX_LEVEL(obj)) && (GET_LEVEL(ch) < LVL_SAINT) )
      send_to_char(ch, "You are too experienced to use that.\r\n");
    else {
...
void perform_remove(struct char_data *ch, int pos, int display)
{
 ...
 /*
  * Added this display int to determine whether or not it should
  * display the remove messages when perform_remove is called
  * this is only used for things popping off right now
  */
  ...
  if (!(obj = GET_EQ(ch, pos)))
     ...
     * Put it here instead of hanlder.c like the equip one
     * so that the stop using messages come before the pop
     * off messages.
     */
     check_size(ch);
...
ACMD(do_claim) {
  ...
  if (!*arg1)
    ...
    send_to_char(ch, "You must claim from a corpse.\r\n");
  else if (!(corpse = get_obj_in_list_vis(ch, arg2, NULL, world[IN_ROOM(ch)].contents))) {
     if (corpse != NULL) {
      if (!(IS_CORPSE(corpse) ) )
          send_to_char(ch, "You must claim from a corpse.\r\n");
       else 
         send_to_char(ch, "There is no corpse here.\r\n");
     }
     else
        send_to_char(ch, "You must claim from a corpse.\r\n");                
  }
  else if (ch->player_specials->last_kill > 0) {
      if (corpse->killer > 0 && corpse->killer == GET_IDNUM(ch) ) {
         if (!(obj = get_obj_in_list_vis(ch, arg1, NULL, corpse->contains)) ) {
            send_to_char(ch, "You can't find that in the corpse.\r\n");
             return;
            }
         if (OBJ_FLAGGED((obj), ITEM_NO_CLAIM) || GET_OBJ_VNUM(obj) < 0) {
            send_to_char(ch, "You can't claim that item.\r\n");
            return;
        }    
        perform_get_from_container(ch, obj, corpse, FIND_OBJ_ROOM);
        ch->player_specials->last_kill = 0;
        act("You claimed $p.", FALSE, ch, obj, 0, TO_CHAR);
        mudlog(NRM, MAX(LVL_SAINT, GET_INVIS_LEV(ch)), TRUE, "(GC) %s claimed %s from %s.",
		      GET_NAME(ch), obj->short_description, corpse->short_description);
      }
      else
      	send_to_char(ch, "You can't claim from that corpse!\r\n");
  }
  else  
    send_to_char(ch, "You can't claim anything!\r\n");     
...
bool trap_check(struct char_data *ch, int location) {
  ...
  char *desc;
  room_rnum to_room;
  int damage, save_value, j;
  bool is_hit;
  
  if(IS_NPC(ch))
    return(FALSE);
    
  for (j = 0; j < MAX_SPELL_AFFECTS; j++) {
    af[j].type = 38;    /* sleep */
    af[j].bitvector = 0;
    af[j].modifier = 0;
    af[j].location = APPLY_NONE;
  }
  
  for (i = world[IN_ROOM(ch)].contents; i; i = i->next_content) {   
      if(GET_OBJ_TYPE(i) == ITEM_TRAP) {   /* dam no save*/
        if(GET_OBJ_VAL(i, 0) == 0) {
          if(location == GET_OBJ_VAL(i, 3) ) {   
            damage = rand_number(GET_OBJ_VAL(i, 1), GET_OBJ_VAL(i, 2));
               if(IS_AFFECTED(ch, AFF_SANCTUARY) )
                  damage /= 2;    
               GET_HIT(ch) -= damage;    
             if ((desc = find_exdesc("TO_VICT", i->ex_description)) != NULL) 
               send_to_char(ch, "%s", desc);
             if ((desc = find_exdesc("TO_ROOM", i->ex_description)) != NULL)            
           ...
           if(GET_HIT(ch) <= 0) {
               die(ch);
                mudlog(BRF, LVL_SAINT, TRUE, "%s killed by %s at %s", GET_NAME(ch), 
                  i->short_description, world[IN_ROOM(ch)].name);
		clanlog(ch, "%s killed by %s.", GET_NAME(ch), i->short_description);
                }
          return(TRUE);
         }
        }
        else if(GET_OBJ_VAL(i, 0) == 1) {  /* dam room no save */
          if(location == GET_OBJ_VAL(i, 3) ) {   
           if ((desc = find_exdesc("TO_VICT", i->ex_description)) != NULL) 
               send_to_char(ch, "%s", desc);
             if ((desc = find_exdesc("TO_ROOM", i->ex_description)) != NULL)            
             ...
             for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room) {
               damage = rand_number(GET_OBJ_VAL(i, 1), GET_OBJ_VAL(i, 2));
               if(IS_AFFECTED(tch, AFF_SANCTUARY) )
                  damage /= 2;   
               GET_HIT(tch) -= damage; 
                  if(GET_HIT(tch) <= 0) {
                    die(tch);
                    mudlog(BRF, LVL_SAINT, TRUE, "%s killed by %s at %s", GET_NAME(tch), 
                      i->short_description, world[IN_ROOM(ch)].name);
		    clanlog(tch, "%s killed by %s.",  GET_NAME(tch), i->short_description);	
                  }
               }
          return(TRUE);
         }
        }           
        else if(GET_OBJ_VAL(i, 0) == 2) { /* dam save 1/2 */
          if(location == GET_OBJ_VAL(i, 3) ) {   
              damage = rand_number(GET_OBJ_VAL(i, 1), GET_OBJ_VAL(i, 2));
              save_value = (rand_number(0, 50)) - GET_DEX(ch) + GET_OBJ_LEVEL(i) - GET_LEVEL(ch);
             if( save_value >= 10) {            
                if ((desc = find_exdesc("TO_VICT", i->ex_description)) != NULL) 
                   send_to_char(ch, "%s", desc);
                if ((desc = find_exdesc("TO_ROOM", i->ex_description)) != NULL)            
                ...
                if ((desc = find_exdesc("SAVE_VICT", i->ex_description)) != NULL) 
                   send_to_char(ch, "%s", desc);
                if ((desc = find_exdesc("SAVE_ROOM", i->ex_description)) != NULL)            
                   for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room) {        
                    if(tch != ch)
                     send_to_char(tch, "%s", desc);
                     }
         ...
         else if(GET_OBJ_VAL(i, 0) == 3) { /* dam save 1/2 room */
          if(location == GET_OBJ_VAL(i, 3) ) {   
           for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room) {
              damage = rand_number(GET_OBJ_VAL(i, 1), GET_OBJ_VAL(i, 2));
              save_value = (rand_number(0, 50)) - GET_DEX(tch) + GET_OBJ_LEVEL(i) - GET_LEVEL(tch);
             if( save_value >= 10) {  
               ...
               GET_HIT(tch) -= damage; 
                  if(GET_HIT(tch) <= 0) {
                    die(tch);
                    mudlog(BRF, LVL_SAINT, TRUE, "%s killed by %s at %s", GET_NAME(tch), 
                      i->short_description, world[IN_ROOM(tch)].name);
		    clanlog(tch, "%s killed by %s.", GET_NAME(tch), i->short_description);	
                  }
            }   
          return(TRUE);
         ...
         else if(GET_OBJ_VAL(i, 0) == 4) { /* dam save 0 */
          if(location == GET_OBJ_VAL(i, 3) ) {   
              damage = rand_number(GET_OBJ_VAL(i, 1), GET_OBJ_VAL(i, 2));
              save_value = (rand_number(0, 50)) - GET_DEX(ch) + GET_OBJ_LEVEL(i) - GET_LEVEL(ch);
             if( save_value >= 10) {            
         ...
         else if(GET_OBJ_VAL(i, 0) == 5) { /* dam save 0 room */
          if(location == GET_OBJ_VAL(i, 3) ) {   
           for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room) {
              damage = rand_number(GET_OBJ_VAL(i, 1), GET_OBJ_VAL(i, 2));
              save_value = (rand_number(0, 50)) - GET_DEX(tch) + GET_OBJ_LEVEL(i) - GET_LEVEL(tch);
        ...
        else if (GET_OBJ_VAL(i, 0) == 6) {  /* teleport */
         if(location == GET_OBJ_VAL(i, 3) )    {
             if ((desc = find_exdesc("TO_VICT", i->ex_description)) != NULL) 
               send_to_char(ch, "%s", desc);
             if ((desc = find_exdesc("TO_ROOM", i->ex_description)) != NULL)            
             ...
             if(GET_OBJ_VAL(i, 2) > 0)
                to_room = rand_number(GET_OBJ_VAL(i, 1), GET_OBJ_VAL(i, 2) );
             else
                to_room = GET_OBJ_VAL(i, 1);  
             to_room = real_room(to_room);
             if (VALID_ROOM_RNUM(to_room)) {                              
                char_from_room(ch);
        ...
        else if (GET_OBJ_VAL(i, 0) == 7) {  /* teleport room*/
         if(location == GET_OBJ_VAL(i, 3) )    {
             if ((desc = find_exdesc("TO_VICT", i->ex_description)) != NULL) 
               send_to_char(ch, "%s", desc);
             if ((desc = find_exdesc("TO_ROOM", i->ex_description)) != NULL)            
             ...
             ch = tch;
                while(tch != NULL) {
                tch = world[IN_ROOM(ch)].people; 
                if (GET_ROOM_VNUM(IN_ROOM(ch)) == GET_OBJ_VAL(i, 1) )
                  return(TRUE);
                if(GET_OBJ_VAL(i, 2) > 0)
                   to_room = rand_number(GET_OBJ_VAL(i, 1), GET_OBJ_VAL(i, 2) );
                else
                    to_room = GET_OBJ_VAL(i, 1);  
                 to_room = real_room(to_room);
                 if((ch = tch->next_in_room) == NULL)  {
                   char_from_room(tch);
                   char_to_room(tch, to_room); 
                   look_at_room(IN_ROOM(tch), tch, 0);                      
                   return(TRUE);
                  }                  
                 if (VALID_ROOM_RNUM(to_room)) {                              
                    char_from_room(tch);
        ...
        else if (GET_OBJ_VAL(i, 0) == 8) {  /* sleep - room */
         if(location == GET_OBJ_VAL(i, 3) )    {
             for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room) {
              is_hit = FALSE;              
             ...
             if (is_hit == TRUE)  {
                af[0].duration = 4 + (GET_OBJ_LEVEL(i) / 4);
                af[0].bitvector = AFF_SLEEP;
                if (GET_POS(tch) > POS_SLEEPING)  {
                  send_to_char(tch, "You feel very sleepy...  Zzzz......\r\n");
                  act("$n goes to sleep.", TRUE, tch, 0, 0, TO_ROOM);
                  GET_POS(tch) = POS_SLEEPING;
                  }
                for (j = 0; j < MAX_SPELL_AFFECTS; j++)
                  if (af[j].bitvector || (af[j].location != APPLY_NONE))
                     affect_join(tch, af+j, FALSE, FALSE, FALSE, FALSE);                 
               }          
            }   
          return(TRUE);
         }
        ...
        else if(GET_OBJ_VAL(i, 0) == 10) {    /* death trap */
          if(location == GET_OBJ_VAL(i, 3) ) {             
             ...
             if ((desc = find_exdesc("TO_ROOM", i->ex_description)) != NULL)            
              ...
              die(ch);
              mudlog(BRF, LVL_SAINT, TRUE, "%s killed by DT %s at %s", GET_NAME(ch), 
                  i->short_description, world[IN_ROOM(ch)].name); 
	      clanlog(ch, "%s entered a Death Room.", GET_NAME(ch));              
          return(TRUE);
         }
        }
        else if(GET_OBJ_VAL(i, 0) == 11) {  /* death trap - room */
          if(location == GET_OBJ_VAL(i, 3) ) {   
           if ((desc = find_exdesc("TO_VICT", i->ex_description)) != NULL) 
               send_to_char(ch, "%s", desc);
             if ((desc = find_exdesc("TO_ROOM", i->ex_description)) != NULL)            
		...
		while (tch->carrying) {
                ...
                for (j = 0; j < NUM_WEARS; j++)
                 if (GET_EQ(tch, j))
                  extract_obj(unequip_char(tch, j));
              die(tch);
              mudlog(BRF, LVL_SAINT, TRUE, "%s killed by DT %s at %s", GET_NAME(tch), 
                  i->short_description, world[IN_ROOM(tch)].name); 
	      clanlog(tch, "%s entered a Death Room.", GET_NAME(tch));     
            }
          return(TRUE);
         }
        }           
...
struct obj_data *obj;
   for (i = 0; i < NUM_WEARS; i++) 
     if (GET_EQ(ch, i)) {
        obj = GET_EQ(ch, i);
      if ( (GET_OBJ_MAX_LEVEL(obj) < GET_LEVEL(ch)) && ((GET_LEVEL(ch) < LVL_SAINT)) ) { 
          act("You are too experienced to use $p any longer and it falls off. ", FALSE, ch, GET_EQ(ch, i), 0, TO_CHAR);
          perform_remove(ch, i, 0);
          obj_from_char(obj);
          obj_to_room(obj, IN_ROOM(ch));
       }
