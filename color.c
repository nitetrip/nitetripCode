...
*                                                                         *
*                                                                         *
*  Edited to version 3.0 by Brandon Allen to be complient                 *
*  with circlemud 3.1 03/02/2003                                          *
*                                                                         *
*  Edited to version 2.2 by (tman@dial.pipex.com) to be complient to      *
*  bpl ?? 4/3/1997                                                        *
*                                                                         *
*  Origional credit to mud@proimages.proimages.com                        *
*                                                                         *
...
void proc_color(char *inbuf, int colour)
{
  ...
  int k, max, c = 0;
  char out_buf[32768], tempbuf[32768];

  if (inbuf[0] == '\0')
    return;

  while (inbuf[j] != '\0') {
   if ((inbuf[j] == '\\') && (inbuf[j + 1] =='c') && (inbuf[j + 2 ] =='0') && (inbuf[j + 3 ] =='l') && !(is_colour(inbuf[j + 4]) == -1)) {
      c = is_colour(inbuf[j + 4]);
      j += 5;
    } else {
      out_buf[p] = inbuf[j];
      j++;
  ...
  strcpy(tempbuf, out_buf);
  sprintf(inbuf, "%s\x1B[0;0m", tempbuf);
