...
void look_in_obj(struct char_data *ch, char *arg)
{
  ...
  if (!*arg)
	...
	case FIND_OBJ_INV:
	  ...
	  list_obj_to_char(obj->contains, ch, SHOW_OBJ_SHORT, TRUE,2);
	  break;
	case FIND_OBJ_ROOM:
	  send_to_char(ch, " (here): \r\n");
	  list_obj_to_char(obj->contains, ch, SHOW_OBJ_SHORT, TRUE,1);
	  break;
	case FIND_OBJ_EQUIP:
...
ACMD(do_score)
{
  ...
  send_to_char(ch, "Your title is %s %s (level %d).\r\n",
...
ACMD(do_who)
{
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if(GET_LEVEL(tch) < LVL_SAINT) { 
      num_can_see++;
      send_to_char(ch, "[%2d %s ] [%s%10s%s] %s %s", GET_LEVEL(tch), CLASS_ABBR(tch), 
      CCCYN(ch, C_SPR), clan_names_output[tch->player_specials->saved.clan_id], 
	        CCNRM(ch, C_SPR),GET_NAME(tch), GET_TITLE(tch));
	  }
	   else
	    {
	      ...
	      num_can_see++;	     
        if(GET_LEVEL(tch) == LVL_SAINT)
          send_to_char(ch, "[%s%s%s] [%s%10s%s] %s %s",
	          CCGRN(ch, C_SPR), goddesc[godnumber], CCNRM(ch, C_SPR), CCCYN(ch, C_SPR), clan_names_output[tch->player_specials->saved.clan_id],
		        CCNRM(ch, C_SPR), GET_NAME(tch), GET_TITLE(tch));
		    else if(GET_LEVEL(tch) == LVL_LIEGE)
          send_to_char(ch, "[%s%s%s] [%s%10s%s] %s %s",
	          CCBGR(ch, C_SPR), goddesc[godnumber], CCNRM(ch, C_SPR), CCCYN(ch, C_SPR), clan_names_output[tch->player_specials->saved.clan_id],
		        CCNRM(ch, C_SPR), GET_NAME(tch), GET_TITLE(tch));
        else if(GET_LEVEL(tch) == LVL_COUNT)  
          send_to_char(ch, "[%s%s%s] [%s%10s%s] %s %s",
	          CCBBL(ch, C_SPR), goddesc[godnumber], CCNRM(ch, C_SPR), CCCYN(ch, C_SPR), clan_names_output[tch->player_specials->saved.clan_id],
		        CCNRM(ch, C_SPR), GET_NAME(tch), GET_TITLE(tch));
 else if(GET_LEVEL(tch) == LVL_DEITY)  
          send_to_char(ch, "[%s%s%s] [%s%10s%s] %s %s",
	          CCBCN(ch, C_SPR), goddesc[godnumber], CCNRM(ch, C_SPR), CCCYN(ch, C_SPR), clan_names_output[tch->player_specials->saved.clan_id],
		        CCNRM(ch, C_SPR), GET_NAME(tch), GET_TITLE(tch));
 else if(GET_LEVEL(tch) == LVL_GOD)  
          send_to_char(ch, "[%s%s%s] [%s%10s%s] %s %s",
	          CCBMG(ch, C_SPR), goddesc[godnumber], CCNRM(ch, C_SPR), CCCYN(ch, C_SPR), clan_names_output[tch->player_specials->saved.clan_id],
		        CCNRM(ch, C_SPR), GET_NAME(tch), GET_TITLE(tch));

		    else if(GET_LEVEL(tch) == LVL_IMPL)  
          send_to_char(ch, "[%s%s%s] [%s%10s%s] %s %s",
	          CCRED(ch, C_SPR), goddesc[godnumber], CCNRM(ch, C_SPR), CCCYN(ch, C_SPR), clan_names_output[tch->player_specials->saved.clan_id],
		        CCNRM(ch, C_SPR), GET_NAME(tch), GET_TITLE(tch));
	    }
...
ACMD(do_attributes)
{
  ...
  sprinttype(ch->player.race, pc_race_types, buf2, sizeof(buf2));
  send_to_char(ch, "Sex: %s, Age: %d, Height: [%d ft %d in], Weight: %d lbs, Race: %s\r\n",
   buf3, GET_AGE(ch), x, y, GET_WEIGHT(ch), buf2);
  sprinttype(ch->player.chclass, IS_NPC(ch) ? npc_class_types : pc_class_types, buf, sizeof(buf));
  send_to_char(ch, "Class: %s, Level: [%2d], XP: [%7d], Align: [%4d]\r\n",
    buf, GET_LEVEL(ch), GET_EXP(ch), GET_ALIGNMENT(ch) );
  if (IS_CLANNED(ch) )
  send_to_char(ch, "Clan: [%10s]    Rank: [ %s ]\r\n", clan_names_output[ch->player_specials->saved.clan_id], 
  	clan_ranks[ch->player_specials->saved.clan_rank]);  
...
/*  
* By level is dumb, but we'll keep it here just in case we change our minds.
*
  ...
  for (dir = 0; dir < NUM_OF_DIRS; dir++) {
    ...
    for (dis = 0; dis <= maxdis; dis++) {
      if (((dis == 0) && (dir == 0)) || (dis > 0)) {
        for (i = world[ch->in_room].people; i; i = i->next_in_room) {
          if ((!((ch == i) && (dis == 0))) && CAN_SEE(ch, i) && !AFF_FLAGGED(i, AFF_HIDE)) {
            sprintf(buf, "%-35.35s: %s%s%s%s", GET_NAME(i), distance[dis],
      ...
      if (EXIT(ch, dir))
			{
      if (EXIT_FLAGGED(EXIT(ch, dir), EX_CLOSED) && (dis != 4))
				{
      if (dis != 0)
		{
		if (!EXIT_FLAGGED(EXIT(ch, dir), EX_HIDDEN))
		 sprintf(buf, "Something BLOCKS your view         : %s%s%s%s",  distance[dis + 1],
                    ((dis > 0) && (dir < (NUM_OF_DIRS - 2))) ? "to the " : "",
                    (dis > 0) ? dirs[dir] : "",
                    ((dis > 0) && (dir > (NUM_OF_DIRS - 3))) ? "wards" : "");
		}
    ...
    {     
     ...
     for (dis = 0; dis <= maxdis; dis++) {
        if (((dis == 0) && (dir == 0)) || (dis > 0)) {
          for (i = world[ch->in_room].people; i; i = i->next_in_room) {
            if ((!((ch == i) && (dis == 0))) && CAN_SEE(ch, i) && dis != 0
