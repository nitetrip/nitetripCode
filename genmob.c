...
int save_mobiles(zone_rnum rznum)
{
  ...
  if ((mobfd = fopen(mobfname, "w")) == NULL) {
    mudlog(BRF, LVL_DEITY, TRUE, "SYSERR: GenOLC: Cannot open mob file for writing.");
    return FALSE;
  }
...
{  
  ...
  sprintasciilong(buf1, AFF2_FLAGS(mob));  
  ...
  for (i = 0; i < MAX_ATTACK_TYPES; i++) {   
    if(mob->char_specials.resist[i] == 1)
      fprintf(fd, "Res%d: %d\n",i, mob->char_specials.resist[i]);   
  }  
  for (i = 0; i < MAX_ATTACK_TYPES; i++) {   
    if(mob->char_specials.vulnerable[i] == 1)
      fprintf(fd, "Vul%d: %d\n",i, mob->char_specials.vulnerable[i]);   
  }  
  for (i = 0; i < MAX_ATTACK_TYPES; i++) {   
    if(mob->char_specials.immune[i] == 1)
      fprintf(fd, "Imm%d: %d\n",i, mob->char_specials.immune[i]);   
  }  
  fputs("E\n", fd);
  return TRUE;
}
...
char buf2[MAX_STRING_LENGTH];
  ...
  sprintasciilong(buf1, MOB_FLAGS(mob));
  sprintasciilong(buf2, AFF_FLAGS(mob));  

  fprintf(fd,	"%s %s %d E\n"
		"%d %d %d %dd%d+%d %dd%d+%d\n",
		buf1, buf2, GET_ALIGNMENT(mob),
		GET_LEVEL(mob), 10 + GET_HITROLL(mob), GET_AC(mob), GET_HIT(mob),
  ...
  fprintf(fd, 	"%d %d\n"
		...
		GET_POS(mob), GET_DEFAULT_POS(mob), GET_SEX(mob), GET_SIZE(mob)
  );
...
void check_mobile_string(mob_vnum i, char **string, const char *dscr)
{
  if (*string == NULL || **string == '\0') {
    ...
    mudlog(BRF, LVL_DEITY, TRUE, smbuf);
    if (*string)
      free(*string);
